...
int main(int argc, char* argv[]) {
    ...
    char header[12] = {0};
    char seqNum[5] = "221zz";
    sprintf(seqNum, "%d",(rand() % (25600 - 0 + 1)) + 0);
    char ackNum[5] = "00000";
    char modSeqNum[6];
    ...
    while(1)
    {
        ...
        bzero(seqNum, 5);
        currentSeqNum = rand() % (25600 - 0 + 1) + 0;
        int fri = sprintf(seqNum, "%d", currentSeqNum);
        int frifri = 5 - fri;
        for (int i = 0; i < frifri; i++)
            strncat(seqNum, &z, 1);
        //fprintf(stderr, "random seq num: %d\n", currentSeqNum);
        bzero(modSeqNum, 6);
        memcpy(modSeqNum, seqNum, 5);
        strncat(modSeqNum, &z, 1);
        ...
        while(1) // wait for client to send first handshake
        {
            ...
            if (buffer[11] == 'b') // syn
            {
                ...
                while(1)
                {
                    ...
                    //change here
                    if (getAck(buffer) == lastSentAck)
                    {
                        fprintf(stderr, "client didn't get last ack\n");
